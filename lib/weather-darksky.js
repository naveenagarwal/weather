...
/*
 ...
 options.time to fetch the weather information for particular day
...
WeatherDarksky.prototype.fetchWeatherForCurrentDay = function(options, callback){
  ...
  // Unix time stamp for current date and time.
  options.time = Math.round(+new Date()/1000);
  weather.getWeatherInformation(options, callback);
};

// fetch the weather information by the day in future up to one week.
